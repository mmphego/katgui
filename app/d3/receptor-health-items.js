        ...
        return {
            ...
            template: [
                '<div ng-repeat="receptor in vm.receptorList track by receptor" ng-if="vm.receptorList.length > 0" ng-switch="vm.mapType" style="display: inline-block" ng-class="{\'in-maintenance-receptor\':getReceptorInMaintenance(receptor)}">',
                    ...
                    '<receptor-health-sunburst-map class="treemap-container" receptor="receptor" ng-right-click="vm.openMenuItems($event)" menu-items="vm.menuItems" ng-switch-when="Sunburst"></receptor-health-sunburst-map>',
                '</div>'
            ].join(''),
            link: function (scope) {
