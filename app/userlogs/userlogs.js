(function() {
    ...
    function UserlogCtrl(MonitorService, $localStorage, $interval, $mdDialog, $scope,
        ...
        vm.chatMode = _.findIndex(vm.inlineTags, {
            name: 'chat'
        }) > -1;
        ...
        vm.setOrderBy = function(name) {
            var index = _.findIndex(vm.orderByFields, {
                value: name
            });
            if (index > -1) {
                vm.orderByFields[index].reverse = vm.orderBy.value === name ? !vm.orderBy.reverse :
                    vm.orderBy.reverse;
        ...
        vm.orderByValueFunction = function(userlog) {
            ...
            return userlog[vm.orderBy.value] ? userlog[vm.orderBy.value] :
                '9999-01-01 00:00:00';
        };
        ...
        vm.addNewInlineLog = function() {
            ...
            if (!vm.newLogStartTimeText) {
                vm.newLogStartTimeText = moment($rootScope.utcDateTime,
                    MOMENT_DATETIME_FORMAT).subtract(5, 'm').format(
        ...
        vm.filterByTag = function(userlog) {
            ...
            for (var i = 0; i < userlog.tags.length; i++) {
                if (_.findIndex(vm.filterTags, {
                        name: userlog.tags[i].name
                    }) > -1) {
                    matchCount++;
                }
            }
        ...
        vm.filterAfterNow = function(userlog) {
            if (vm.orderBy.value === 'start_time' && vm.orderBy.reverse) {
                return userlog.start_time > vm.newLogStartTimeText || (!vm.newLogStartTimeText ||
                    vm.newLogStartTimeText.length === 0);
            } else if (vm.orderBy.value === 'start_time') {
                return userlog.start_time <= vm.newLogStartTimeText || (!vm.newLogStartTimeText ||
                    vm.newLogStartTimeText.length === 0);
            } else if (vm.orderBy.value === 'end_time' && vm.orderBy.reverse) {
                return userlog.end_time > vm.newLogEndTimeText || userlog.end_time.length ===
                    0;
            } else if (vm.orderBy.value === 'end_time') {
        ...
        vm.filterBeforeNow = function(userlog) {
            if (vm.orderBy.value === 'start_time' && vm.orderBy.reverse) {
                ...
                return userlog.end_time && userlog.end_time <= vm.newLogEndTimeText || (!vm
                    .newLogEndTimeText || vm.newLogEndTimeText.length === 0);
            } else if (vm.orderBy.value === 'end_time') {
                return userlog.end_time > vm.newLogEndTimeText || userlog.end_time.length ===
                    0;
            }
        ...
        vm.editUserLog = function(userlog, event) {
            UserLogService.editUserLog(userlog, $rootScope.currentUser.id === userlog.user_id,
                'userlogDialogContentElement', event);
        };
        ...
        vm.afterInit = function() {
            UserLogService.listTags().then(function() {
                ...
                UserLogService.listUserLogsForTimeRange(start, end).then(function() {
                    vm.lastQueryDayStart = vm.lastQueryDayStart.subtract(1,
                        'M').subtract(1, 'd');
                    vm.lastQueryDayEnd = vm.lastQueryDayEnd.subtract(1, 'd');
                    vm.lastQueryDayTextStart = vm.lastQueryDayStart.format(
                        'YYYY-MM-DD');
                    vm.lastQueryDayTextEnd = vm.lastQueryDayEnd.format(
                        'YYYY-MM-DD');
                    vm.lastFutureQueryDayStart = vm.lastFutureQueryDayStart
                        .add(1, 'd');
                    vm.lastFutureQueryDayTextStart = vm.lastFutureQueryDayStart
                        .format('YYYY-MM-DD');
                    vm.lastFutureQueryDayEnd = vm.lastFutureQueryDayEnd.add(
                        1, 'M').add(1, 'd');
                    vm.lastFutureQueryDayTextEnd = vm.lastFutureQueryDayEnd
                        .format('YYYY-MM-DD');
                });
                if ($stateParams.action === 'add') {
                    ...
                    UserLogService.getUserLogById($stateParams.id).then(function(
                        result) {
                        ...
                        var start_time = $stateParams.startTime ?
                            $stateParams.startTime : userlog.start_time;
                        var end_time = $stateParams.endTime ? $stateParams.endTime :
                            userlog.end_time;

                        if ($rootScope.currentUser.id === userlog.user_id &&
                            $stateParams.content) {
                            content = $stateParams.content.replace(/\\n/g,
                                '\n'); // preserve newlines
                        }
        ...
        if ($rootScope.utcDateTime) {
            vm.newLogStartTimeText = moment($rootScope.utcDateTime, MOMENT_DATETIME_FORMAT).subtract(
                5, 'm').format(MOMENT_DATETIME_FORMAT);
            vm.verifyInlineInputs();
        } else {
            vm.undbindutcDateTimeSet = $rootScope.$on('utcDateTimeSet', function(event, value) {
                vm.newLogStartTimeText = moment(value, MOMENT_DATETIME_FORMAT).subtract(
                    5, 'm').format(MOMENT_DATETIME_FORMAT);
                vm.verifyInlineInputs();
            });
        }
        ...
        vm.fetchMoreLogs = function() {
            ...
            var end = vm.lastQueryDayEnd.format('YYYY-MM-DD 23:59:59');
            UserLogService.listUserLogsForTimeRange(start, end).then(function() {
                vm.lastQueryDayStart = vm.lastQueryDayStart.subtract(1, 'M').subtract(
                    1, 'd');
                vm.lastQueryDayEnd = vm.lastQueryDayEnd.subtract(1, 'M').subtract(1,
                    'd');
                vm.lastQueryDayTextStart = vm.lastQueryDayStart.format('YYYY-MM-DD');
                vm.lastQueryDayTextEnd = vm.lastQueryDayEnd.format('YYYY-MM-DD');
        ...
        vm.fetchMoreFutureLogs = function() {
            ...
            UserLogService.listUserLogsForTimeRange(start, end).then(function() {
                vm.lastFutureQueryDayStart = vm.lastFutureQueryDayStart.add(1, 'M')
                    .add(1, 'd');
                vm.lastFutureQueryDayTextStart = vm.lastFutureQueryDayStart.format(
                    'YYYY-MM-DD');
                vm.lastFutureQueryDayEnd = vm.lastFutureQueryDayEnd.add(1, 'M').add(
                    1, 'd');
                vm.lastFutureQueryDayTextEnd = vm.lastFutureQueryDayEnd.format(
                    'YYYY-MM-DD');
        ...
        vm.setInlineStartTimeBeforeLog = function(userlog) {
            ...
            if (vm.orderBy.value === 'start_time' && vm.orderBy.reverse) {
                vm.newLogStartTimeText = moment(userlog.start_time, MOMENT_DATETIME_FORMAT)
                    .subtract(1, 's').format(MOMENT_DATETIME_FORMAT);
            } else if (vm.orderBy.value === 'start_time') {
                vm.newLogStartTimeText = moment(userlog.start_time, MOMENT_DATETIME_FORMAT)
                    .add(1, 's').format(MOMENT_DATETIME_FORMAT);
            } else if (vm.orderBy.value === 'end_time' && vm.orderBy.reverse) {
                vm.newLogStartTimeText = moment(userlog.end_time, MOMENT_DATETIME_FORMAT).subtract(
                    5, 'm').format(MOMENT_DATETIME_FORMAT);
                vm.newLogEndTimeText = moment(userlog.end_time, MOMENT_DATETIME_FORMAT).subtract(
                    1, 's').format(MOMENT_DATETIME_FORMAT);
            } else if (vm.orderBy.value === 'end_time') {
                vm.newLogStartTimeText = moment(userlog.end_time, MOMENT_DATETIME_FORMAT).subtract(
                    5, 'm').format(MOMENT_DATETIME_FORMAT);
                vm.newLogEndTimeText = moment(userlog.end_time, MOMENT_DATETIME_FORMAT).add(
                    1, 's').format(MOMENT_DATETIME_FORMAT);
            }
        ...
        vm.checkForMandatoryInlineTag = function() {
            ...
            vm.filterTags.forEach(function(tag) {
                if (UserLogService.mandatoryTagsList.indexOf(tag.name.toLowerCase()) >
                    -1) {
